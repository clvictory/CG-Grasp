...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, input_channels=4, num_classes=1,
                 embed_dim=48, depths=[2, 2, 2, 2], depths_decoder=[1, 2, 2, 2], num_heads=[1,2,4,8],
        ...
        if use_prototype_fusion:
            ...
            # 确定融合层级
            if fusion_levels == 's4_only':
                self.fusion_targets = [3]  # 只融合s4层
            elif fusion_levels == 'high':
                self.fusion_targets = [2, 3]  # 融合s3和s4
            elif fusion_levels == 'all':
                self.fusion_targets = [0, 1, 2, 3]  # 融合所有层
            else:
                raise ValueError(f"Unsupported fusion_levels: {fusion_levels}")

            # 各层特征维度
            dims = [int(embed_dim * 2 ** i) for i in range(self.num_layers)]  # e.g., [48,96,192,384]
            self.embed_dims = dims

            # 投影头：将各层特征映射到统一维度（取最高层维度的一半）
            self.prototype_dim = dims[3] // 2
            self.projection_heads = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(dims[i], self.prototype_dim, kernel_size=1),
            ...
            # 原型感知卷积：用于融合的卷积，输入为特征通道+原型维度
            self.prototype_aware_convs = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(dims[i] + self.prototype_dim, dims[i], kernel_size=3, padding=1),
                    nn.BatchNorm2d(dims[i]),
                    nn.ReLU(inplace=True)
                ) for i in range(4) if i in self.fusion_targets
            ])
            # 建立融合层级到卷积模块的映射索引
            self.fusion_conv_indices = {level: idx for idx, level in enumerate(self.fusion_targets)}
            ...
            # 记忆库相关（按层级存储，不考虑类别，可扩展为按类别）
            self.prototype_memory = {}      # key: "level_{level}"
            self.cluster_centers = {}       # key: "level_{level}"
            self.update_counter = {}         # key: "level_{level}"
    ...
    # =================== 原型聚类融合辅助方法 ===================
    def extract_prototypes(self, features, level):
        """
        从指定层级的特征图中提取实例原型
        ...
        """
        # 生成注意力图（使用L2范数）
        attention_map = torch.norm(features, dim=1, keepdim=True)  # [B,1,H,W]
        # 二值掩码
        ...
        for i in range(B):
            if binary_mask[i].sum() > 0:
                ...
                # 截断到投影维度
                proto = proto[:self.prototype_dim]
            else:
                proto = features[i].mean(dim=(1, 2))[:self.prototype_dim]
            prototypes.append(proto)
        return torch.stack(prototypes)

    def update_cluster_memory(self, prototypes, level):
        """更新指定层级的记忆库"""
        memory_key = f"level_{level}"
        ...
        # 存入新原型（detach并转cpu numpy）
        for proto in prototypes.detach().cpu().numpy():
            self.prototype_memory[memory_key].append(proto)
        self.update_counter[memory_key] += 1
        if self.update_counter[memory_key] % 50 == 0:
            self.update_cluster_centers(memory_key)

    def get_context_prototypes(self, current_prototypes, level):
        """检索上下文原型（聚类中心加权平均）"""
        memory_key = f"level_{level}"
        ...
        # 余弦相似度
        curr_norm = F.normalize(current_prototypes, p=2, dim=1)
        center_norm = F.normalize(cluster_centers, p=2, dim=1)
        sim = torch.matmul(curr_norm, center_norm.t())  # [B, num_clusters]
        topk_sim, topk_idx = torch.topk(sim, min(self.top_k, len(cluster_centers)), dim=1)
        weights = F.softmax(topk_sim, dim=1)  # [B, topk]
        selected = cluster_centers[topk_idx]  # [B, topk, D]
        weighted = torch.sum(selected * weights.unsqueeze(-1), dim=1)  # [B, D]
        return weighted

    def update_cluster_centers(self, memory_key):
        """对指定记忆库执行k-means更新聚类中心"""
    ...
    def apply_progressive_fusion(self, features_list, prototypes):
        """
        渐进式融合主函数
        features_list: 长度为4的列表，每个元素为 [B, C, H, W] 的特征图（按从低到高 s1~s4）
        prototypes: 当前批次从s4提取的原型 [B, D]
        返回增强后的特征列表（同样形状）
        """
        enhanced = list(features_list)
        fusion_targets = sorted(self.fusion_targets, reverse=True)  # 从高到低

        for level_idx, level in enumerate(fusion_targets):
            feat = enhanced[level]
            B, C, H, W = feat.shape
            # 获取该层级的上下文原型
            context = self.get_context_prototypes(prototypes, level)
            if context is None:
                continue
            # 计算强度
            if self.adaptive_fusion:
                strength = self.fusion_strength * (0.8 ** level_idx)
            else:
                strength = self.fusion_strength
            # 广播到空间
            context_feat = context.unsqueeze(-1).unsqueeze(-1).expand(B, -1, H, W)

            if level in [2, 3]:  # 高层使用完整融合（有卷积）
                if level in self.fusion_conv_indices:
                    conv_idx = self.fusion_conv_indices[level]
                    conv = self.prototype_aware_convs[conv_idx]
                    combined = torch.cat([feat, context_feat], dim=1)
                    fused = conv(combined)
                else:
                    fused = feat + strength * context_feat
            else:  # 低层轻量融合
                if self.adaptive_fusion and level in self.fusion_conv_indices:
                    w = self.fusion_weights[self.fusion_conv_indices[level]]
                    fused = feat + w * context_feat
                else:
                    fused = feat + strength * context_feat

            # 残差连接
            enhanced[level] = feat + strength * fused
    ...
    def forward(self, x, class_id=0):
        """
        class_id: 用于区分不同类别的原型记忆（当前未使用类别区分，保留接口）
        """
        # 编码器
        x, x_downsample = self.forward_features(x)  # x: [B, L, C]; x_downsample: list of [B, L_i, C_i]

        if self.use_prototype_fusion:
            # 将序列特征转换为图像特征 [B, C, H, W]
            features_2d = []
            HWs = []
            for i, feat in enumerate(x_downsample):
                ...
                assert L == H * W, f"Shape mismatch at level {i}: {L} vs {H*W}"
                feat_2d = feat.permute(0, 2, 1).view(B, C, H, W)
                features_2d.append(feat_2d)
                HWs.append((H, W))

            # 提取s4的原型（最高层）
            s4_prototypes = self.extract_prototypes(features_2d[3], level=3)

            if self.training:
                # 更新所有融合层级的记忆库
                for level in self.fusion_targets:
                    proto = self.extract_prototypes(features_2d[level], level=level)
                    self.update_cluster_memory(proto, level)

            # 进行渐进式融合
            enhanced_2d = self.apply_progressive_fusion(features_2d, s4_prototypes)

            # 将增强的特征转回序列格式，替换x_downsample
            for i, feat_2d in enumerate(enhanced_2d):
                B, C, H, W = feat_2d.shape
                feat_seq = feat_2d.view(B, C, H * W).permute(0, 2, 1)  # [B, L, C]
                x_downsample[i] = feat_seq

            # 最后一层特征x也需要用增强后的s4替换，并重新应用norm
            x = enhanced_2d[3]  # [B, C, H, W]
            B, C, H, W = x.shape
    ...
    def compute_loss(self, xc, yc, is_train=True, use_prior=True, class_id=0):
        """
        增加class_id参数传递给forward（如果需要）
        """
        y_pos, y_cos, y_sin, y_width = yc
        pos_pred, cos_pred, sin_pred, width_pred = self(xc, class_id=class_id)
