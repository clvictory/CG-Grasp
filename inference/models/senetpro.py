...
class SEResUNet(GraspModel):
    def __init__(self, input_channels=4, output_channels=1, channel_size=32, dropout=False, prob=0.0,
                 ...
                 fusion_levels=[0, 1, 2, 3, 4],  # 融合层级: 0=conv2, 1=conv3, 2=seres2, 3=seres3, 4=seres4
        ...
        if self.use_multi_level_fusion:
            # 定义5个关键层级：conv2输出, conv3输出, seres2输出, seres3输出, seres4输出
            self.feature_levels = 5
            level_dims = [
            ...
            for i, level_dim in enumerate(level_dims):
                if i in self.fusion_levels:
                    ...
                    self.prototype_aware_convs.append(
                        nn.Sequential(
                            nn.Conv2d(level_dim + level_dims[i] // 2, level_dim, kernel_size=3, padding=1),
    ...
    def forward(self, x_in, get_cluster=False, class_id=0):
        ...
        x0 = x
        intermediate_features.append(x0)
        ...
        # SERes4模块（瓶颈层）
        x = self.seres4(x)
        if 4 in self.fusion_levels and self.use_multi_level_fusion:
            x = self.apply_prototype_fusion(x, level_idx=4, class_id=class_id)
        intermediate_features.append(x)
    ...
    def get_cluster_statistics(self, class_id=0):
        ...
        for level in self.fusion_levels:
            ...
            if memory_key in self.prototype_memory:
                ...
                cluster_centers = 0 if self.cluster_centers[memory_key] is None else len(
                    self.cluster_centers[memory_key])
                stats[f"level{level}"] = {
