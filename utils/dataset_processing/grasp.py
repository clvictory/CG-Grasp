...
class GraspRectangles:
    ...
    def load_from_graspnet_file(cls, fname, scale=1.0):
        arr = np.load(fname)
        # print(len(arr) - 1)
        sample_indices = np.linspace(0, len(arr) - 1, 200, endpoint=False).astype(int)
        arr = arr[sample_indices]
        # print(len(arr))
        grs = []
        for a in arr:
            vec = np.array([a[2] - a[0], a[3] - a[1]])
            x, y = a[0], a[1]
            w = np.linalg.norm(vec)
            # h = np.linalg.norm(np.cross(vec, [0, 1])) / np.linalg.norm(vec)
            h = a[4]
    ...
    @classmethod
    def load_from_vmrd_file(cls, fname, scale=1.0):
        grs = []
        with open(fname) as f:
            for l in f:
                grs.append([float(v) for v in l[:-1].split(' ')[:8]])
                # index based on row, column (y,x), and the Jacquard dataset's angles are flipped around an axis.
        grs = np.array(grs)
        grs_lable = points2label(grs)
        grs_cvt = []
        for g in grs_lable:
            x, y, w, h, theta = g
            grs_cvt.append(Grasp(np.array([y, x]), -theta / 180.0 * np.pi, w, h).as_gr)
    ...
    def draw(self, shape, position=True, angle=True, width=True):
        ...
        # print(width_out.shape, width_out.mean(), width_out.max(), width_out.min())
...
class GraspRectangle:
    ...
    def scale(self, factor):
        ...
        try:
            if factor == 1.0:
                return
            point_t = self.points.astype(np.float32)
            point_t *= factor
            self.points = point_t.astype(np.int64)
            # self.points *= factor
        except np.core._exceptions.UFuncTypeError as e:  
            print(type(factor), factor, type(self.points), self.points)
    ...
    def plot(self, ax, color=None):
        ...
        ax.plot(points[:, 1], points[:, 0], color='white', linewidth='8')
...
class Grasp:
    ...
    def max_iou(self, grs, angle_threshold=np.pi / 6):
        """
        ...
        max_iou = 0
        for gr in grs:
            iou = self_gr.iou(gr, angle_threshold=angle_threshold)
