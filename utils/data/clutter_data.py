...
class ClutterDataset(GraspDatasetBase):
    """
    ...
    def __init__(self, file_path, ds_rotate=0, **kwargs):
        ...
        """
        super(ClutterDataset, self).__init__(**kwargs)

        self.grasp_files = glob.glob(os.path.join(file_path, "02", 'pcd*cpos.txt'))
        ...
        self.rgb_files = [f.replace('d.png', 'r.png') for f in self.depth_files]
    ...
    def get_rgb(self, idx, rot=0, zoom=1.0, normalise=True):
        ...
        # print("1", rgb_img.shape)
        center, left, top = self._get_crop_attrs(idx)
        rgb_img.rotate(rot, center)
        rgb_img.crop((top, left), (min(480, top + self.output_size), min(640, left + self.output_size)))
        # print("2", rgb_img.shape, (top, left), (min(480, top + self.output_size), min(640, left + self.output_size)))
        rgb_img.zoom(zoom)
